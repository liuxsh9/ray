...
cdef class GcsClient:
    ...
    @_auto_reconnect
    def get_all_job_info(self, job_ids=[], submission_ids=[],
        ...
        cdef:
            int64_t timeout_ms = round(1000 * timeout) if timeout else -1
            c_vector[c_string] c_job_ids
            c_vector[c_string] c_submission_ids
        ...
        if job_ids:
            for job_id in job_ids:
                if job_id:
                    c_job_ids.push_back(job_id)

        if submission_ids:
            for submission_id in submission_ids:
                if submission_id:
                    c_submission_ids.push_back(submission_id)

        with nogil:
            check_status(self.inner.get().GetAllJobInfo(
                c_job_ids, c_submission_ids, timeout_ms, c_job_infos))
